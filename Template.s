...
Start
		...
		;LDR R1, =GPIO_PORTL_DATA_R		;Stores the addresss of Port L's data register in R1
		;LDR R0, [R1]					;Dereferences addresss of Port L's data register and puts value at R0
		;AND R0,R0, #0x01				;Checking if Bit PL0 is 1
		;CMP R0, #1						;if PL0 is 1
		MOV R2, #0
		LDR R3, =1000000
		B	keep_on						;keep LED on
		;BNE turn_off					;turn LED off
		
		

keep_on 
		LDR R1, =GPIO_PORTF_DATA_R ;Stores the address of the target register in R1 
		LDR R0, [R1] ;Dereferences R1 to put the contents of the target register in R0 
		ORR R0,R0, #0x10 ;Modifies the contents of R0 as needed 
		STR R0, [R1] ;Stores the new value back into the target register 
		ADD R2, #1
...
turn_off
		LDR R1, =GPIO_PORTF_DATA_R ;Stores the address of the target register in R1 
		LDR R0, [R1] ;Dereferences R1 to put the contents of the target register in R0 
		AND R0,R0, #0xEF ;Modifies the contents of R0 as needed 
		STR R0, [R1] ;Stores the new value back into the target register
		ADD R2, #1
		CMP R2, R3
		BNE turn_off
